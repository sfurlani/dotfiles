...
k () {
  ...
  do
    ...
    SIZELIMITS_TO_COLOR=(
      ...
      1048576  10   # <= 1mb || 512kb
      )
    typeset -i ANCIENT_TIME_COLOR=8  # > more than 2 years old
    typeset -a FILEAGES_TO_COLOR
    FILEAGES_TO_COLOR=(
             0 1  # < in the future, #spooky
            60 15  # < less than a min old
          3600 7  # < less than an hour old
    ...
    do
        ...
        if (( INSIDE_WORK_TREE == 0 )); then
          ...
          if (( IS_DIRECTORY )); then
            if command git --git-dir="$GIT_TOPLEVEL/.git" --work-tree="${NAME}" diff --stat --quiet --ignore-submodules HEAD &>/dev/null # if dirty
              then REPOMARKER=$'\e[38;5;2m|\e[0m' # Show a green vertical bar for clean
              else REPOMARKER=$'\e[0;1m+\e[0m' # Show a red vertical bar if dirty
            fi
          fi
        else
          if (( IS_DIRECTORY )); then
          ...
          STATUS=${STATUS[1,2]}
            if [[ $STATUS == ' M' ]]; then REPOMARKER=$'\e[0;2m+\e[0m';     # Tracked & Dirty
          elif [[ $STATUS == 'M ' ]]; then REPOMARKER=$'\e[38;5;2m+\e[0m'; # Tracked & Dirty & Added
          elif [[ $STATUS == '??' ]]; then REPOMARKER=$'\e[38;5;1m?\e[0m'; # Untracked
          elif [[ $STATUS == '!!' ]]; then REPOMARKER=$'\e[38;5;8m|\e[0m'; # Ignored
          elif [[ $STATUS == 'A ' ]]; then REPOMARKER=$'\e[38;5;3m+\e[0m'; # Added
          else                             REPOMARKER=$'\e[38;5;2m|\e[0m'; # Good
          fi
        fi
      fi
      ...
      if   [[ $IS_SYMLINK           == 1 ]]; then NAME=$'\e['"$K_COLOR_LN"'m'"$NAME@"$'\e[0m';
...
_k_bsd_to_ansi() {
  local foreground=$1 background=$2 foreground_ansi background_ansi foreground_bold
  case $foreground in
    A|B|C|D|E|F|"G"|"H") foreground_bold="01";;
    *) foreground_bold=0;;
  esac
  case $foreground in
    A|a) foreground_ansi=30;;
    B|b) foreground_ansi=31;;
    C|c) foreground_ansi=32;;
    D|d) foreground_ansi=33;;
    E|e) foreground_ansi=34;;
    F|f) foreground_ansi=35;;
    "G"|g) foreground_ansi=36;;
    "H"|h) foreground_ansi=37;;
    x) foreground_ansi=0;;
  esac
  case $background in
  ...
  # echo "$foreground $background -> $foreground_bold;$background_ansi;$foreground_ansi\n"
  printf "%s;%s;%s" $background_ansi $foreground_bold $foreground_ansi
}
